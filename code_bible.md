...
- **Unified Session History**:
    ...
    - **Local Video Player**: Use a full-screen, solid black \`LocalVideoPlayerDialog\` with \`DialogProperties(usePlatformDefaultWidth = false)\` to completely obscure underlying main UI controls and dashboard buttons during match review playback. Features built-in ExoPlayer controls and a top header bar with a close button.
...
- **TV Supplementary Experience (PickleView TV)**:
    ...
    - **Detailed Playback Diagnostic Banners**: When stream or replay playback errors occur, `VideoPlayerLayer` MUST parse the `PlaybackException` cause (e.g. H.265/HEVC decoder unsupported, network timeout, local HTTP error) and render a high-visibility diagnostic banner (`StreamErrorBanner`) with clear troubleshooting instructions.
